...
type RequestHandler = Arc<
    dyn Fn(AdminRequest) -> std::pin::Pin<Box<dyn std::future::Future<Output = AdminResponse> + Send>>
        + Send
        + Sync,
>;
...
impl AdminServer {
    pub fn new(
        ...
        let parquet_api = ParquetApi::new(engine.clone());
        let sql_api = SqlApi::new(engine.clone());
        let lifecycle_api = LifecycleApi::new(engine.clone());
...
impl BoundAdminServer {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        service_mgr.ensure_default_service(
            "default",
            50051,
            "default",
            "./data",
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        ...
                        let mut mgr = metrics_collector.write().await;
                        ...
                        if let Err(e) = engine_for_writer.write_batch(&sys_metrics) {
                            tracing::debug!("write sys metrics: {}", e);
                        }
        ...
        shutdown_rx2.changed().await.map_err(|_| AdminError::Nng("shutdown channel closed".into()))?;
            ...
            Box::pin(async move {
                match req {
                    ...
                    AdminRequest::ServiceCreate {
                        name, port, config, data_dir, engine: eng, enable_iceberg,
                    } => match service_mgr
                        .create_service(&name, port, &config, &data_dir, &eng, enable_iceberg)
                        .await
                    {
                        Ok(info) => AdminResponse::ok(serde_json::to_value(&info).unwrap_or_default()),
                    ...
                    AdminRequest::ServiceLogs { name, lines } => {
                        ...
                        match service_mgr.get_service_logs(&name, log_lines).await {
                            Ok(logs) => AdminResponse::ok(serde_json::to_value(&logs).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e.to_string()),
                        }
                    }
                    ...
                    AdminRequest::ConfigGetProfile { name } => {
                        ...
                        match mgr.get_profile(&name) {
                            Ok(p) => AdminResponse::ok(serde_json::to_value(&p).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e.to_string()),
                        }
                    }
                    ...
                    AdminRequest::ConfigCompare { profile_a, profile_b } => {
                        let mgr = config_mgr.read().await;
                    ...
                    AdminRequest::TestSql { service: _, sql } => {
                        let runner = TestRunner::new(engine.clone());
                    ...
                    AdminRequest::TestWriteBench {
                        ...
                        let runner = TestRunner::new(engine.clone());
                        match runner.run_write_bench(&measurement, total_points, workers, batch_size).await {
                            Ok(result) => {
                                AdminResponse::ok(serde_json::to_value(&result).unwrap_or_default())
                            }
                            Err(e) => AdminResponse::err(e.to_string()),
                    ...
                    AdminRequest::CollectorConfigure { interval_secs, enabled } => {
                        let mgr = metrics.read().await;
                    ...
                    AdminRequest::RocksdbCfDetail { name } => {
                        ...
                        match mgr.rocksdb_cf_detail(&name) {
                            Some(detail) => AdminResponse::ok(serde_json::to_value(&detail).unwrap_or_default()),
                    ...
                    AdminRequest::RocksdbCompact { cf } => {
                        ...
                        match mgr.rocksdb_compact(cf.as_deref()) {
                            Ok(()) => AdminResponse::ok(serde_json::json!({"compacted": true, "cf": cf})),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    AdminRequest::RocksdbKvScan { cf, prefix, start_key, limit } => {
                        let mgr = metrics.read().await;
                        match mgr.rocksdb_kv_scan(&cf, prefix.as_deref(), start_key.as_deref(), limit.unwrap_or(20)) {
                            Ok(result) => AdminResponse::ok(serde_json::to_value(&result).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    AdminRequest::RocksdbKvGet { cf, key } => {
                        ...
                        match mgr.rocksdb_kv_get(&cf, &key) {
                            Ok(entry) => AdminResponse::ok(serde_json::to_value(&entry).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    AdminRequest::RocksdbSeriesSchema => {
                        ...
                        match mgr.rocksdb_series_schema() {
                            Ok(schema) => AdminResponse::ok(serde_json::to_value(&schema).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    ...
                    AdminRequest::ParquetPreview { path, limit } => {
                        match parquet_api.preview(&path, limit.unwrap_or(50)) {
                            Ok(preview) => AdminResponse::ok(serde_json::to_value(&preview).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }

                    AdminRequest::SqlExecute { sql } => {
                        match sql_api.execute(&sql).await {
                            Ok(result) => AdminResponse::ok(serde_json::to_value(&result).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    AdminRequest::SqlTables => {
                        match sql_api.tables().await {
                            Ok(tables) => AdminResponse::ok(serde_json::to_value(&tables).unwrap_or_default()),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                    ...
                    AdminRequest::LifecycleArchive { older_than_days } => {
                        match lifecycle_api.archive(older_than_days) {
                            Ok(archived) => AdminResponse::ok(serde_json::json!({"archived": archived})),
                            Err(e) => AdminResponse::err(e),
                        }
                    }
                }
