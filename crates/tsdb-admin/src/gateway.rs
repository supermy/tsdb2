...
use nng::{Message as NngMessage, Socket as NngSocket, Protocol};
...
pub struct GatewayState {
    ...
    ws_clients: Arc<RwLock<Vec<(uuid::Uuid, tokio::sync::mpsc::Sender<String>)>>>,
}
...
impl GatewayState {
    ...
    fn dial_with_retry(sock: &NngSocket, url: &str, label: &str) -> std::result::Result<(), AdminError> {
        let mut attempts = 0;
        let max_attempts = 50;
        loop {
            match sock.dial(url) {
    ...
    pub async fn send_request(&self, request: AdminRequest) -> std::result::Result<AdminResponse, String> {
        ...
        let result = tokio::task::spawn_blocking({
            ...
            move || {
                ...
                let response: AdminResponse =
                    serde_json::from_slice(reply.as_slice()).map_err(|e| format!("parse: {}", e))?;
                Ok(response)
            }
...
pub fn build_router(state: GatewayState) -> Router {
    ...
    let api = Router::new()
        ...
        .route("/api/services/:name", get(get_service).delete(delete_service))
    ...
    let static_files = tower_http::services::ServeDir::new(&dashboard_dir)
        .fallback(tower_http::services::ServeFile::new(dashboard_dir.join("index.html")));
...
macro_rules! api_handler {
    ($state:expr, $req:expr) => {
        match $state.send_request($req).await {
            ...
            Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(AdminResponse::err(e))),
        }
    };
}
...
async fn create_service(
    ...
    api_handler!(state, AdminRequest::ServiceCreate {
        name: body.name,
        port: body.port,
        config: body.config,
...
async fn get_service(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ServiceStatus { name })
}

async fn delete_service(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
...
async fn start_service(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ServiceStart { name })
}

async fn stop_service(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ServiceStop { name })
}

async fn restart_service(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ServiceRestart { name })
}

#[derive(Deserialize)]
struct LogsQuery { lines: Option<usize> }
...
async fn get_service_logs(
    ...
    api_handler!(state, AdminRequest::ServiceLogs { name, lines: query.lines })
}
...
async fn get_config(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
...
async fn save_config(
    ...
    api_handler!(state, AdminRequest::ConfigSaveProfile { name: body.name, content: body.content })
}

#[derive(Deserialize)]
struct SaveConfigBody { name: String, content: String }

async fn delete_config(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    if let Err(e) = validate_name(&name) {
        return (StatusCode::BAD_REQUEST, Json(AdminResponse::err(e)));
    }
...
async fn apply_config(State(state): State<GatewayState>, Json(body): Json<ApplyConfigBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ConfigApply { service: body.service, profile: body.profile })
}

#[derive(Deserialize)]
struct ApplyConfigBody { service: String, profile: String }

async fn compare_config(State(state): State<GatewayState>, Json(body): Json<CompareConfigBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ConfigCompare { profile_a: body.profile_a, profile_b: body.profile_b })
}

#[derive(Deserialize)]
struct CompareConfigBody { profile_a: String, profile_b: String }

async fn test_sql(State(state): State<GatewayState>, Json(body): Json<TestSqlBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::TestSql { service: body.service, sql: body.sql })
}

#[derive(Deserialize)]
struct TestSqlBody { service: String, sql: String }

async fn test_write_bench(State(state): State<GatewayState>, Json(body): Json<TestWriteBenchBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::TestWriteBench {
        service: body.service, measurement: body.measurement,
        total_points: body.total_points, workers: body.workers, batch_size: body.batch_size,
    })
}

#[derive(Deserialize)]
struct TestWriteBenchBody { service: String, measurement: String, total_points: usize, workers: usize, batch_size: usize }

async fn test_read_bench(State(state): State<GatewayState>, Json(body): Json<TestReadBenchBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::TestReadBench {
        service: body.service, measurement: body.measurement, queries: body.queries, workers: body.workers,
    })
}

#[derive(Deserialize)]
struct TestReadBenchBody { service: String, measurement: String, queries: usize, workers: usize }

async fn metrics_health(State(state): State<GatewayState>, Query(query): Query<MetricsHealthQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::MetricsHealth { service: query.service })
}

#[derive(Deserialize)]
struct MetricsHealthQuery { service: String }

async fn metrics_stats(State(state): State<GatewayState>, Query(query): Query<MetricsStatsQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::MetricsStats { service: query.service.unwrap_or_else(|| "default".to_string()) })
}

#[derive(Deserialize)]
struct MetricsStatsQuery { service: Option<String> }

async fn metrics_timeseries(State(state): State<GatewayState>, Query(query): Query<MetricsTimeseriesQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::MetricsTimeseries {
...
#[derive(Deserialize)]
struct MetricsTimeseriesQuery { service: Option<String>, metric: Option<String>, range_secs: Option<u64> }

async fn metrics_alerts(State(state): State<GatewayState>, Query(query): Query<MetricsAlertsQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::MetricsAlerts { service: query.service })
}

#[derive(Deserialize)]
struct MetricsAlertsQuery { service: String }

async fn collector_status(State(state): State<GatewayState>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::CollectorStatus)
}

async fn collector_configure(State(state): State<GatewayState>, Json(body): Json<CollectorConfigureBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::CollectorConfigure {
...
#[derive(Deserialize)]
struct CollectorConfigureBody { interval_secs: u64, enabled: bool }
...
async fn rocksdb_cf_detail(State(state): State<GatewayState>, Path(name): Path<String>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::RocksdbCfDetail { name })
}

async fn rocksdb_compact(State(state): State<GatewayState>, Json(body): Json<RocksdbCompactBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::RocksdbCompact { cf: body.cf })
}

#[derive(Deserialize)]
struct RocksdbCompactBody { cf: Option<String> }
...
async fn rocksdb_kv_scan(State(state): State<GatewayState>, Query(query): Query<KvScanQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::RocksdbKvScan {
        cf: query.cf,
        prefix: query.prefix,
...
async fn rocksdb_kv_get(State(state): State<GatewayState>, Query(query): Query<KvGetQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::RocksdbKvGet { cf: query.cf, key: query.key })
}
...
#[derive(Deserialize)]
struct ParquetFileDetailQuery { path: String }

async fn parquet_file_detail(State(state): State<GatewayState>, Query(query): Query<ParquetFileDetailQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ParquetFileDetail { path: query.path })
}

#[derive(Deserialize)]
struct ParquetPreviewQuery { path: String, limit: Option<usize> }

async fn parquet_preview(State(state): State<GatewayState>, Query(query): Query<ParquetPreviewQuery>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::ParquetPreview { path: query.path, limit: query.limit })
}

#[derive(Deserialize)]
struct SqlExecuteBody { sql: String }

async fn sql_execute(State(state): State<GatewayState>, Json(body): Json<SqlExecuteBody>) -> impl IntoResponse {
...
#[derive(Deserialize)]
struct LifecycleArchiveBody { older_than_days: u64 }

async fn lifecycle_archive(State(state): State<GatewayState>, Json(body): Json<LifecycleArchiveBody>) -> impl IntoResponse {
    api_handler!(state, AdminRequest::LifecycleArchive { older_than_days: body.older_than_days })
}
